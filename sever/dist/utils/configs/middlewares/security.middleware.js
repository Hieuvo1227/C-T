 ...
 */
const checkInjectionForNoSQL = (value) => {
    if (typeof value === "object" && value !== null) {
        for (const key in value) {
            ...
            if (typeof value[key] === "object") {
                if (checkInjectionForNoSQL(value[key]))
                    return true;
            }
        }
    }
 ...
 */
export const injectionGuard = (req, res, next) => {
    const hasInjection = deepCheck(req.body) || deepCheck(req.query) || deepCheck(req.params);
    if (hasInjection) {
        return res
            .status(400)
 ...
 * Cấu hình CORS
 */
const ALLOWED_ORIGIN = CLIENT_URL.split(',').map(url => url.trim());
